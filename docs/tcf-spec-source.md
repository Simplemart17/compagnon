...
**ARCHITECTURAL SUCCESSOR DONE — closed by Story 11-1 on 2026-05-11.** Story 10-7's minimum-viable bridge (strip emoji + markdown decoration from the Correction Report block; preserve the regex contract) is superseded. The Correction Report text block is removed entirely from `buildConversationPrompt` and replaced with a `## Correction Reporting (Tool-Call)` block that instructs the model to invoke `report_correction({ original, corrected, explanation, category })` whenever it detects an error. Corrections now arrive via the new `report_correction` Realtime tool-call (`src/hooks/use-realtime-voice.ts` `handleFunctionCall` branch + `src/lib/schemas/ai-responses.ts` `reportCorrectionArgsSchema` validator + `src/lib/realtime-corrections.ts` `processReportCorrectionCall` + `drainPendingCorrections` pure helpers + a per-turn `pendingToolCorrectionsRef` buffer drained when the terminal `response.output_audio_transcript.done` fires and force-cleared on `response.done` + `case "error"`). The brittle regex `/"([^"]+)"\s*→\s*"([^"]+)"\s*\(([^)]+)\)/g` and the `inferCategory` keyword-matching heuristic are both deleted (Story 10-2 "delete don't alias" pattern). The model emits no correction text on the audio side at all; the TTS-reads-asterisks failure mode is structurally impossible because the prompt block that instructed the model to emit asterisks no longer exists. Audit findings P1-6 (silent zero-correction failures from regex brittleness) and P2-1 (voice-mode emoji formatting) both close architecturally. The `TranscriptView.getDisplayText` legacy sentinel stripper at [src/components/conversation/TranscriptView.tsx](../src/components/conversation/TranscriptView.tsx) is retained as a forward-compat / backward-compat shim for pre-11-1 stored transcripts in `conversation_messages` (rendered via the conversation history screen at [app/(tabs)/conversation/history.tsx](<../app/(tabs)/conversation/history.tsx>)). The speaking-score formula at `src/lib/speaking-score.ts` `computeSpeakingScore` (extracted from the hook for testability + future-tuning baseline) is unchanged; its INPUT (`correctedEntries`) is more accurate post-11-1, so the same conversation produces a slightly lower (more accurate) score than pre-11-1 — correctness improvement, not regression.
