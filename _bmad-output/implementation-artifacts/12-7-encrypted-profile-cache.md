...
| 2026-05-13 | Story 12-7 implementation complete. New `src/lib/secure-cache.ts` (240 lines) wraps `expo-secure-store` with TTL-aware `{data, timestamp, ttlMs}` envelope: `setSecureCache` / `getSecureCache` / `invalidateSecureCache` / `clearSecureCacheForUser` / `buildSecureKey` pure helper + `readSecureCacheIgnoreTTL` (for `cacheWithFallback` fallback-read path) + `readLegacyPlaintextEntry` + `deleteLegacyPlaintextEntry` migration helpers + `__resetSecureCacheForTests` test-only stub. iOS pinned to `WHEN_UNLOCKED_THIS_DEVICE_ONLY`. Platform-fallback for web. Error handling via `captureError` + `addBreadcrumb({feature: "secure-cache-{set,get}-fail"})`. `src/lib/cache.ts`: new `SECURE_CACHE_KEYS: ReadonlySet<string> = new Set([CACHE_KEYS.PROFILE])` allowlist; routing fork at `getCache` / `setCache` / `invalidateCache`; one-shot migration block in `getCache` fires `secure-cache-migrated` breadcrumb once per user per device; `cacheWithFallback` catch-branch forks for secure keys via `readSecureCacheIgnoreTTL`; `clearUserCache` sweeps both stores. 26 new Jest cases across 2 test files. Test count 1393 → 1419 (+26 net; exceeded spec target ~1413). 3 new Sentry feature tags (all 21 chars). All 5 quality gates green. CLAUDE.md updated with Story 12-7 architecture paragraph. Story 9-3 / 9-4 / 9-6 / 9-10 / 12-1 / 12-2 / 12-3 / 12-4 / 12-5 / 12-6 invariants preserved by construction (encryption fork is internal to `cache.ts`; 6 consumer files compile unchanged). Closes audit P1-11 architecturally. |
| 2026-05-13 | Story 12-7 review-round-1 patches applied (HIGH × 4 + MED × 8 + LOW × 9). HIGH: P1 migration TTL re-check moved BEFORE `setSecureCache` + breadcrumb (pre-patch wrote expired data + fired breadcrumb as success); P2 module-level `migrationInFlight: Map<userId+key, Promise>` gate serializes concurrent `getCache` calls (breadcrumb-once contract); P3 `cacheWithFallback` catch-branch second fallback to `readLegacyPlaintextEntry` for offline cold-launch users; P4 envelope shape validation across all 3 reader helpers via new `isValidEnvelope` helper (prevents `undefined-as-fresh` for non-envelope JSON). MED: P5 Case 5 explicit `setItemAsync` data-argument assertion; P6 `deleteLegacyPlaintextEntry` AWAITED before breadcrumb (idempotency under delete-fail); P7 `clearUserCache` SecureStore-first order (sign-out semantics: destroy encrypted PII first); P8 `readLegacyPlaintextEntry`/`deleteLegacyPlaintextEntry` `@internal` JSDoc + `buildLegacyCacheKey`/`LEGACY_PLAINTEXT_CACHE_PREFIX` made module-private; P9 Case 17 added — end-to-end production flow `setCache` → `cacheWithFallback`; P10 JSDoc documents iOS-passcode-required caveat; P11 `clearSecureCacheForUser` wrapped in try/catch + `captureError("cache-clear-user-secure")`; P12 `buildSecureKey` throws on empty + sanitizes invalid charset via `sanitizeSecureKeySegment`. LOW: P13 `__resetSecureCacheForTests` JSDoc clarified test-isolation comes from mock `__reset()` not this stub; P14 CLAUDE.md "8 consumers" → "6 consumer files"; P15 module size "~240" → "~311 lines"; P16 dead exports removed (covered by P8); P17 grammar fix "Module-level no state" → "Module is stateless"; P18 `cache-secure-routing.test.ts` beforeEach Platform.OS reset; P19 spec AC #2 divergence documented (implementation inlines fork via `readSecureCacheIgnoreTTL` — TTL semantic preservation); P20 Case 13 reframed to LITERAL-STRING regression check + balanced-paren regex; P21 Case 9 cleanup assertion. +18 net Jest cases (1419 → 1437): Cases 14-18 in routing file + Cases 14a-g (7 parametric) + 15-20 in secure-cache file. All 5 quality gates green. CLAUDE.md updated with `Story 12-7 review-round-1 patches` paragraph. |
