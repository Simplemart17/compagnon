...
- 2026-05-13 — Story 12-8 review-round-1 patches (HIGH × 3 + MED × 6 + LOW × 10 = 19 total). HIGH: P1 always-merge client validator results in `mapSupabaseWeakPasswordError` (when `password` is provided) / P2 strength meter heuristic tightened (entropy + very-short clauses) / P3 drift detector balanced-paren walker. MED: P4 template-literal French message + placeholder / P5 trim-before-length whitespace-padding defense / P6 strict drift-detector placeholder regex / P7+P8 generic French fallback for any `weak_password` / P9 a11y duplicate-announce fix. LOW: P11 `name`-or-`code` defense-in-depth / P12 `\n` join / P13 4 missing JSDocs / P14 CLAUDE.md size claims corrected / P15 type-import test / P16 Arabic-Indic digit test / P17 no-space Cyrillic variant / P18 runbook curl email + deletion required / P19 single useMemo / P20 parametric non-array reasons. +19 net Jest cases (1473 → 1492). All 5 quality gates green. Cross-story invariants verified clean.
- 2026-05-13 — Story 12-8 review-round-2 patches (HIGH × 3 + MED × 3 + LOW × 8 = 14 total). HIGH: R2-P1 trim-before-signUpWithEmail closes round-trip mismatch (round-1 P5 caused the bug it claimed to fix — Supabase bcrypts raw input, no server-side trim); R2-P2 `isWeakPasswordError` tightened to reject incoherent `{code,name}` shapes (round-1 P11 OR was too permissive); R2-P3 drift detector walker now string-literal-aware (round-1 P3 missed `)` inside quoted strings). MED: R2-P4 PasswordStrengthIndicator child Text uses `importantForAccessibility="no"` not `"no-hide-descendants"` (Android leaf-hide semantics fixed); R2-P5 `computePasswordStrengthLabel` signature collapsed to `(reasons, password)` — derived `password.length` internally, eliminates desync hazard; R2-P6 in-place CLAUDE.md size correction + P11 categorization arithmetic fix. LOW: R2-P7 Unicode-whitespace JSDoc note / R2-P8 parametric whitespace coverage / R2-P9 trailing-period symmetry on French messages / R2-P10 useMemo JSDoc honesty / R2-P11 walker JSDoc tightened / R2-P12 ordering drift detector / R2-P13 placeholder regex Prettier-tolerant / R2-P14 P1 description reword. +11 net Jest cases (1492 → 1503). All 5 quality gates green. Cross-story invariants verified clean.
