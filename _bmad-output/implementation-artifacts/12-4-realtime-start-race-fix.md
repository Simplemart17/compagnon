...
- [ ] **MIGRATE** the 13 `this.session?.foo(...)` call sites inside `handleEvent`-reachable paths to `this.safeSessionCall(s => s.foo(...), "context-tag")`:
  - 10 `sendFunctionResult` sites in `handleFunctionCall` + sub-handlers → `context: "tool-call-{kind}"` (e.g., `tool-call-save-vocabulary`, `tool-call-note-error-pattern`, `tool-call-report-correction`).
...
- [ ] **Story 11-1 tool-call protocol holds** — 10 `sendFunctionResult` sites migrated to the helper with no behavior change in the happy path.
...
- [x] **Task 2: Introduce `safeSessionCall` helper + migrate 13 call sites** (AC #2)
  - [x] Added private `safeSessionCall<T>(fn, context)` method (Story 12-2 wrapper-with-breadcrumb pattern).
  - [x] Migrated 10 `sendFunctionResult` sites with 5 per-tool context tags: `tool-call-save-vocabulary` (3 sites), `tool-call-note-error-pattern` (2 sites), `tool-call-report-correction` (4 sites), `tool-call-unknown` (1 site), `tool-call-handler-error` (1 site).
...
- **P2-21 race architecturally closed**: `this.session` populated before any WebSocket event can fire; the 13 `safeSessionCall` sites preserve happy-path bit-identical behavior while making the silent-no-op failure mode observable in Sentry.
...
- **One new Sentry feature tag**: `"orchestrator-session-null-on-event"` (34 chars; under 80-char allowlist threshold; `context` extras key already allowlisted).
...
- `src/lib/realtime-orchestrator.ts` — reordered `this.session = session` (lines 1331-1342) + added `safeSessionCall` helper (lines 367-401) + migrated 13 call sites.
...
| 2026-05-13 | Story 12-4 implementation complete. `src/lib/realtime-orchestrator.ts`: reordered `this.session = session` to BEFORE `await session.connect()` + inner try/catch with `this.session = null` + `isAiSpeakingMirror = false` + `responseInFlight = false` cleanup on connect failure; added `safeSessionCall<T>(fn, context)` private helper (Story 12-2 wrapper-with-breadcrumb pattern); migrated 13 `this.session?.method()` call sites in `handleEvent`-reachable paths to `safeSessionCall` invocations with 8 distinct context tags (5 tool-call + 2 barge-in + 1 audio-stream); 4 public-API entry points intentionally NOT migrated. New test file `src/lib/__tests__/realtime-orchestrator-session-race.test.ts` (10 Jest cases: 5 drift-detector with comment-stripped source + 1 connect-failure-cleanup + 2 helper-behavior + 2 Story-11-1/11-2-preservation). Test count 1333 → 1343 (+10; spec target was ~1343, exact). One new Sentry feature tag `"orchestrator-session-null-on-event"` (34 chars; under 80 threshold). Story 9-3 / 9-4 / 9-5 / 9-6 / 9-7 / 9-8 / 9-9 / 9-10 / 10-X / 11-1 (tool-call protocol) / 11-2 (barge-in + reconnect) / 11-3 / 11-4 / 11-5 / 11-6 / 11-7 / 11-8 / 12-1 (orchestrator structure) / 12-2 / 12-3 invariants preserved by construction. CLAUDE.md updated. All 5 quality gates green. |
| 2026-05-13 | Story 12-4 review-round-1 patches applied: HIGH × 2 (P1 audio-stream restructure — `isConnected` check moved INSIDE the `safeSessionCall` closure so the null-breadcrumb path is reachable; pre-patch the outer `this.session?.isConnected` pre-gate made it structurally unreachable dead code; P2 documentation count drift fix — "14 sites" → "13", "11 sendFunctionResult" → "10", "6 tool-call tags" → "5", "7 distinct" → "8", "36 chars" → "34") + MED × 8 (P3 connect-failure adds `session.disconnect({reason:"user"})` before nulling — tears down half-open WebSocket + handler wiring; P4 Case 6 direct null-probe via post-failure event dispatch + `setImmediate` drain to prevent `persistConversation` test-boundary leak; P5 `safeSessionCall` TOCTOU capture-then-check; P6 Case 9 setup comment corrected — no `response.created` handler exists, OR-gate uses `inflightItemId` arm via `response.output_audio_transcript.delta`; P7 `session.on(handler)` BEFORE `this.session = session` so re-entrant code paths see fully-wired session; P8 `session.on()` wrapped in same try/catch as await for synchronous-throw defense; P9 JSDoc documents the sync-fn invariant — async fn would silently drop the Promise; P10 drift-detector regex fragility — `[\s\S]*?` separators for formatter resilience + new Case 3b/3c pinning P3 disconnect + P7+P8 ordering invariants) + LOW × 5 (P11 "7 → 8 distinct context tags"; P12 "36 → 34 chars"; P13 breadcrumb `message` from categorical-duplicate to human-readable "Session ref null when handler dispatched", `feature` extras key keeps categorical Sentry tag; P14 test `user` cast `as User` not `as never`; P15 subsumed by P2). +3 net Jest cases (1343 → 1346): Case 3b + Case 3c drift-detector pins (P3+P7+P8 invariants) + Case 6b runtime disconnect-on-failure case. **Deferred (4 pre-existing):** partial mirror reset in inner catch; shallow `Object.freeze` `getState()` (Story 12-1 P15); `start()` public-API `sendText` narrowing (out-of-scope); `session.on()` unsubscribe closure discarded (partially addressed by P3). **5 rejected** as noise: ESM-future-fragility speculation; barge-in-truncate closure pattern correct; Case 8 `end()` vs `dispose()` coupling intentional; partial-mirror-reset (deferred); breadcrumb `feature` vs `message` (covered by P13). All 5 quality gates green. CLAUDE.md updated with the review-round-1 patches paragraph. |
