...
| P3-9 | No item bank caching — every exercise regenerated by GPT (cost + no anti-repetition) **✓ Closed by Story 10-8 (2026-05-10) — basic per-user dedupe via `question_stem_hashes` column + retry-once contract; server-side item bank cache + cross-user sharing remain Epic 13.x scope** | `src/hooks/use-exercise.ts` |
...
- 10.8 Anti-cheat & frequency anti-repetition — add basic item dedupe (hash of question stem) to avoid showing the same generated MCQ across sessions. **DONE — closed by Story 10-8 (2026-05-10).** Per-user hash-set against the user's last 100 completed exercises at the same `(skill, cefr_level)`; retry-once-at-temp-0.6 if filtered question count < `MIN_FRESH_QUESTIONS_PER_SKILL`; on retry-exhausted accept the dup + Sentry breadcrumb. Shared djb2 hash helper extracted to `src/lib/text-hash.ts` so Story 9-5 voice transcript dedup and Story 10-8 exercise dedup share one source of truth. Speaking-task 3-day-bucket selector (Story 9-8) NOT touched.
