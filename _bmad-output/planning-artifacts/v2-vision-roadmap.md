...
- **19.1 Curriculum spine (A1→B2)** ✅ (A1→B2 COMPLETE 2026-07-20 — the D-C1 scope target fully met; C1-C2 are the deferred "advanced tracks") (slices 1-3 2026-07-19: **A1 COMPLETE** — 6 units / 30 lessons / 309 vocab items (Se présenter · La famille · Au quotidien · La nourriture · En ville · Le temps libre); slice 4 2026-07-20: **A2 Units 1-3** — 15 lessons / 122 vocab items (Hier et avant · Les courses et les achats · La santé) — passé composé (avoir + être + agreement + negation), vouloir/pouvoir/devoir paradigms, object pronouns, numbers 100-1000, avoir mal à, full reflexive paradigm, il faut advice; slice 5 2026-07-20: **A2 Units 4-6** — 15 lessons / 109 vocab items (Projets et rendez-vous · Souvenirs et habitudes · La ville et le monde) — futur proche + indirect object pronouns lui/leur + pronoun y, the **imparfait** (formation + imparfait-vs-passé-composé contrast + habitual markers), comparatives/superlatives + meilleur, expanded directions + ordinals + country prepositions; **A2 is now COMPLETE (6 units / 30 lessons / 231 vocab)** — the spine covers A1→A2 fully (12 units / 60 lessons / 540 vocab); slice 6 2026-07-20: **B1 Units 1-3** — 15 lessons / 98 vocab items (Demain je ferai… · Si j'avais… · Ce que je pense) — the **futur simple** (regular + 7 irregular stems + quand/dès que + futur), the **conditionnel présent** (futur-stem + imparfait-endings bridge + politeness + si-clauses hypotheticals), **relative pronouns** qui/que/où + opinion verbs + the now-unlocked contrast connectors cependant/pourtant/donc/alors; **B1 is now STARTED (3 of ~6 units)** — the spine covers A1→A2 + B1-opening (15 units / 75 lessons / 638 vocab); the 19-3 placement fall-down now enters B1 placements at the B1 start (B2+ fall down to B1); remaining: B1 Units 4-6 → B2); slice 7 2026-07-20: **B1 Units 4-6** — 15 lessons / 99 vocab items (Avant c'était déjà fait · Celui que je préfère · Il faut que je parte) — the **plus-que-parfait** (past-before-past + 3-tense narration + venir de recent-past chunk), the pronouns **en / dont / celui-celle** + double object pronouns le-lui, and **the subjonctif présent** (formation + il faut que / vouloir que / emotion triggers) as the B1 capstone; **B1 is now COMPLETE (6 units / 30 lessons / 197 vocab)** — the spine covers A1→B1 fully (18 units / 90 lessons / 737 vocab); slice 8 2026-07-20: **B2 Units 1-3** — 15 lessons / 93 vocab items (Argumenter et nuancer · Il a dit que… · En faisant, je comprends) — the formal **argumentation connectors** (en effet / par conséquent / en revanche / d'une part-d'autre part + alors que/tandis que/malgré) + expanded subjunctive triggers pour que / bien que, the **discours indirect** (reported speech with the présent→imparfait / PC→plus-que-parfait / futur→conditionnel tense shifts), and the **gérondif** (en + present participle, same nous-stem as the imparfait) + the indefinite relatives ce qui / ce que / ce dont; **B2 is now STARTED (3 of ~6 units)** — the A1→B2 span is now populated at every level (21 units / 105 lessons / 830 vocab), reaching the D-C1 scope target at the B2 threshold; the 19-3 placement fall-down now enters B2 placements at the B2 start (C1/C2 fall down to B2); remaining: B2 Units 4-6 for full A1→B2 depth); slice 9 2026-07-20: **B2 Units 4-6** — 15 lessons / 92 vocab items (J'aurais aimé… · C'est bien connu · Avant que tout change) — the **conditionnel passé** + **si + plus-que-parfait** past hypotheticals (the third conditional + j'aurais dû/pu regret chunks), the **voix passive** (être + participle agreeing with the subject, across tenses, + « on » as the active alternative), and the **expanded subjunctive triggers** (avant que / jusqu'à ce que / à condition que / sans que / il est possible que / je ne pense pas que) as the B2 capstone; **B2 is now COMPLETE (6 units / 30 lessons / 185 vocab) and the FULL A1→B2 spine is done: 24 units / 120 lessons / 922 vocab** — the D-C1 "A1→B2 deep first" scope target is fulfilled; remaining: C1-C2 advanced tracks (deferred)) — CEFR can-do-statement-based syllabus: ~6 units/level × ~5 lessons/unit, each lesson = can-do outcome + grammar target + vocab set (tier-aligned with Story 10-4) + conversation scenario. Authored as versioned content files (JSON/MD in-repo), drafted with `french-pedagogy-expert`, **human-reviewed by the operator** (who is conveniently also a learner). ~2 weeks authoring for A1–A2, iterate upward.
...
- **19.4 Curated item bank** 🚧 (slice 1 2026-07-20: **engine shipped + A1 Unit 1 banked** — versioned item-bank format (`src/content/item-banks/<lessonId>.json`, reusing the drill's `mcqQuestionSchema`), a throw-loud loader, and `use-lesson-drill` now PREFERS the bank (instant, zero-AI, zero-repetition) with live-generation fallback for un-banked lessons; A1 Unit 1's 5 lessons ship 45 reviewed items (9/lesson). Remaining: author banks for the other 115 lessons — pure content follow-ups on the shipped engine) — generated-then-reviewed exercise items stored + versioned per lesson (replaces infinite fresh generation on the curriculum path). Cuts cost, kills repetition risk, and creates the substrate Epic 20 calibrates.
