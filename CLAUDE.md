...
**TCF scoring pipeline (per-skill, publisher-anchored):** post-Epic-10.2, scoring is split into per-skill conversion functions in `src/lib/scoring.ts`: `rawPercentToListeningReadingScore(rawPercent, "listening" | "reading")` returns a 0–699 TCF score for the QCM skills; `rawPercentToWritingSpeakingScore(rawPercent)` returns a 0–20 publisher score for the production-task skills. Band boundaries are anchored to the IRCC CLB equivalency table at `src/lib/ircc-bands.ts` `IRCC_CLB_BANDS` (sourced verbatim from `docs/tcf-spec-source.md §2.2`; CLB 7 is the Express Entry threshold). The legacy 7-band linear `rawToTCFScore` is **deleted**, not aliased — every caller is updated. `calculateCompositeScore` was renamed to `calculateInternalCompositeForUI` and JSDoc-flagged as NOT IRCC-equivalent and NOT used by the promotion engine; `SKILL_WEIGHTS_TCF_CANADA` (4-skill, equal 0.25 weights for listening/reading/writing/speaking) replaces the prior 5-skill `SKILL_WEIGHTS` and explicitly excludes Grammar — Grammar stays in the `TCFSkill` union as a non-TCF practice skill per operator decision (`docs/tcf-spec-source.md §10` follow-up #1). Speaking pipeline (`src/lib/speaking-mock-test-persist.ts`) now persists `mock_tests.total_score` in publisher-scale 0–20 for `test_type="speaking"` via the new `computeSpeakingScore0to20` helper in `src/lib/speaking-scoring.ts` (`composite / 5`); pre-10-2 historical rows hold legacy 0–699 values (discontinuity documented but not backfilled — Epic 17.1 owns schema versioning). `cefrLevelFromWritingSpeakingScore` in `scoring.ts` maps 0–20 → CEFR via CLB equivalence for the persisted `cefr_result`. UI-labeling `CEFR_LEVELS.tcfScoreMin/Max` round-number bands stay for the self-assessment-grid display; IRCC math uses `IRCC_CLB_BANDS`. `app/(tabs)/mock-test/results.tsx:60` literal `500` replaced with `TCF.C1_MIN`. The promotion engine (`src/lib/activity.ts` `evaluatePromotion`, story 9-2 contract) still uses the internal 0–100 `skill_progress.score` scale (intentional UX-soft semantics; switching to IRCC bands is a deferred follow-up). Regression-tested in `src/lib/__tests__/ircc-bands.test.ts`, `scoring.test.ts`, `speaking-scoring.test.ts`, `speaking-mock-test-persist.test.ts`, plus the Story 10-2 "promotion does not consume composite" block in `activity.test.ts`. Verified 2026-05-10, story 10-2.
