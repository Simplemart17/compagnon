...
describe("Story 13-1 — orchestrator render-storm drift detectors (audit P2-3)", () => {
  // Story 18-4 R1: hard-interrupt arms route through onAiOutputInterrupted(),
  ...
  it("Case 6: `cancelPendingAiTextRaf` is called inside `dispose()`", () => {
    ...
    expect(body).toMatch(/this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/);
  });
  ...
  it("Case 7: `cancelPendingAiTextRaf` is called from every `.done` / error / barge-in / reconnect path", () => {
    ...
    expect(audioDoneArm).toMatch(/this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/);

    const textDoneArm = extractCaseArm('case "response.output_text.done":');
    expect(textDoneArm).toMatch(/this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/);
    ...
    expect(audioTranscriptDoneArm).toMatch(
      /this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/
    );
    ...
    expect(handleErrorBody).toMatch(
      /this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/
    );

    // handleSpeechStarted (barge-in path)
    const bargeInBody = extractMethodBody("private handleSpeechStarted(): void {");
    expect(bargeInBody).toMatch(/this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/);
    ...
    expect(reconnectingBody).toMatch(
      /this\.(cancelPendingAiTextRaf|onAiOutputInterrupted)\s*\(\s*\)/
    );
  });
